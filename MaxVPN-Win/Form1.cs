...
namespace MaxVPN_Win
{
    public partial class Form1 : Form
    {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            string result = await SendCommandAsync("connect", wireGuardConfig);
        ...
        private async Task<string> SendCommandAsync(string command, WireGuardConfig config = null)
        {
            try
            {
                ...
                using (var pipeClient = new NamedPipeClientStream(".", pipeName, PipeDirection.InOut, PipeOptions.Asynchronous))
                {
                    ...
                    {
                        string payload = config != null ? JsonSerializer.Serialize(config) : null;
